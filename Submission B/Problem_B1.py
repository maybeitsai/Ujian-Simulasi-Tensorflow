...
def solution_B1():
    ...
    model = keras.Sequential([
        tf.keras.layers.Dense(units=17, input_shape=[1], activation='linear'),
        tf.keras.layers.Dense(units=1)
    ])

    model.summary()

    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0178), loss='mean_squared_error')
